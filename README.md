...
### Permission denied on /home/green/.self

`make first-up` and `make up` now auto-fix host directory ownership for `uid:gid=1001:1001`.

If you still hit permission issues, run:

```bash
set -a; . ./.env; set +a
sudo chown -R 1001:1001 "$SELF_HOME" "$SELF_APP_HOME"
ls -ld "$SELF_HOME" "$SELF_APP_HOME"
docker compose run --rm backend sh -lc "id && test -w /home/green/.self && echo writable"
```
